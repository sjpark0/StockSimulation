...
fn main(){
    ...
    let mut price_histories: HashMap<String, Vec<f64>> = HashMap::new();
    ...
    let mut portfolio_vec :Vec<RebalancePortfolio> = threshold.iter().map(|x| RebalancePortfolio::new(initial_capital, &ticker_fraction, fee_rate, *x)).collect();

    let mut cnt_strategy = 1;
    for p in portfolio_vec.iter_mut(){
        for duration in vec_duration.iter(){
            let tmp_rolling = p.rolling_return(&price_histories, *duration);
    ...
    let mut buy_and_hold = BuyHoldPortfolio::new(initial_capital, fee_rate, &ticker_fraction);
    for duration in vec_duration.iter(){
        let tmp_rolling = buy_and_hold.rolling_return(&price_histories, *duration);
        let max_indices = tmp_rolling.sorting_final_capital().remove_redundant_from_maximum();
        let min_indices = tmp_rolling.sorting_final_capital().remove_redundant_from_minimum();
